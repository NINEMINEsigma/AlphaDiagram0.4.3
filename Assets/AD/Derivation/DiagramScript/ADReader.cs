...
namespace AD.Utility
{
    public abstract class ADReader : System.IDisposable
    {
        ...
        public virtual ADReaderPropertyEnumerator Properties
        {
            get
            {
                return new ADReaderPropertyEnumerator(this);
            }
        }

        internal virtual ADReaderRawEnumerator RawEnumerator
        {
            get
            {
                return new ADReaderRawEnumerator(this);
            }
        }

        public string FullPath { get; private set; }
        public bool typeChecking { get; private set; }
        ...
        {
            return Read<T>(ADType.GetOrCreateADType(typeof(T)));
        }
        ...
        {
            ReadInto<T>(obj, ADType.GetOrCreateADType(typeof(T)));
        }
        ...
        {
            return ReadProperty<T>(ADType.GetOrCreateADType(typeof(T)));
        }

        [System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]
        public T ReadProperty<T>(ADType type)
        {
            ReadPropertyName();
            return Read<T>(type);
        }
        ...
        {
            if (!Goto(key))
                throw new KeyNotFoundException("Key \"" + key + "\" was not found in file \"" + FullPath + "\". Use Load<T>(key, defaultValue) if you want to return a default value if the key does not exist.");

            Type type = ReadTypeFromHeader<T>();

            T obj = Read<T>(ADType.GetOrCreateADType(type));
        ...
        {
            if (!Goto(key))
                throw new KeyNotFoundException("Key \"" + key + "\" was not found in file \"" + FullPath + "\"");

            Type type = ReadTypeFromHeader<T>();

            ReadInto<T>(obj, ADType.GetOrCreateADType(type));

            //ReadKeySuffix(); //No need to read key suffix as we're returning. Doing so would throw an error at this point for BinaryReaders.
        }

        protected virtual void ReadObject<T>(object obj, ADType type)
        {
            // Check for null.
            if (StartReadObject())
                return;
        ...
        public class ADReaderPropertyEnumerator
        {
            public ADReader reader;

            public ADReaderPropertyEnumerator(ADReader reader)
            {
                this.reader = reader;
            }
        ...
        [EditorBrowsable(EditorBrowsableState.Never)]
        public class ADReaderRawEnumerator
        {
            public ADReader reader;

            public ADReaderRawEnumerator(ADReader reader)
            {
                this.reader = reader;
            }
    ...
    public class ADJSONReader : ADReader
    {
