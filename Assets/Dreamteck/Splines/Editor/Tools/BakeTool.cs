    ...
    public class BakeTool : SplineTool
    {
        ...
        public override void Draw(Rect windowRect)
        {
            ...
            if (GUILayout.Button("Bake"))
            {
                if (saveMesh)
                {
                    savePath = EditorUtility.OpenFolderPanel("Save Directory", Application.dataPath, "parent");
                    ...
                    {
                        EditorUtility.DisplayDialog("Save error", "OBJ files can be saved outside of the project parent only when \"Save without baking\" is selected. Please select a directory inside the project in order to save.", "OK");
                        return;
                    }
